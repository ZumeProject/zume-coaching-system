...
class Zume_Query_Cumulative extends Zume_Queries_Base {
    ...
    public static function countries( $stages = [ 0,1,2,3,4,5,6 ], $end_date = null, $trend = false, $negative = false ) {
        // transient cache
        $request_key = hash( 'md5', serialize( __METHOD__ . serialize( $stages ) . $end_date . $trend . $negative ) );
        $cached = get_transient( $request_key );
        if ( $cached ) {
        ...
        $list = self::countries_list_cumulative( $stages, $end_date, $trend, $negative );
    ...
    public static function countries_list_cumulative( $stages = [ 0,1,2,3,4,5,6 ], $end_date = null, $trend = false, $negative = false ) {
        global $wpdb;
        $world_grid_ids = self::world_grid_id_sql();
        if ( is_null( $end_date ) ) {
            $end_date = time();
