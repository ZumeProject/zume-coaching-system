...
abstract class Zume_Goals_Chart_Base
{

    public $core = 'zume-goals';
    ...
    public function __construct() {
        ...
        if ( strpos( $url_path, 'zume-goals' ) === 0 ) {
            if ( !$this->has_permission() ){
                return;
            }
            add_filter( 'dt_metrics_menu', [ $this, 'base_menu' ], 20 ); //load menu links

            if ( strpos( $url_path, "zume-goals/$this->base_slug" ) === 0 ) {
        ...
        <script>
            jQuery(document).ready(function($) {
                ...
                window.site_info = {
                    ...
                    'rest_root': 'zume_goals/v1/',
                    'site_url': '<?php echo site_url(); ?>',
                    'rest_url': '<?php echo esc_url_raw( rest_url() ); ?>',
                    'total_url': '<?php echo esc_url_raw( rest_url() ); ?>zume_goals/v1/total',
                    'range_url': '<?php echo esc_url_raw( rest_url() ); ?>zume_goals/v1/range',
                    'map_url': '<?php echo esc_url_raw( rest_url() ); ?>zume_goals/v1/map',
                    'list_url': '<?php echo esc_url_raw( rest_url() ); ?>zume_goals/v1/list',
                    'elements_url': '<?php echo esc_url_raw( rest_url() ); ?>zume_goals/v1/training_elements',
                ...
                window.template_single_list = ({key, valence, label, value, description}) => {
                    return `
                        <div class="grid-x">
                        <div class="cell z-card zume-goals-list ${key} ${valence} hover">
                            <div class="z-card-single">
                ...
                window.template_single_map = ({key, valence, label, value, description}) => {
                    return `
                        <div class="grid-x">
                        <div class="cell z-card zume-goals-map ${key} ${valence} hover">
                            <div class="z-card-single">
                ...
                window.template_trio = ({key, link, label, goal, goal_valence, goal_percent, trend, trend_valence, trend_percent, value, description}) => {
                    ...
                    <div class="grid-x">
                      <div class="cell z-card  ${key}">
                          <div class="z-card-main zume-goals-list ${hover} ${key}" >
                              <div class="z-card-title ${key}">
                                  ${label}
                              </div>
                              <div class="z-card-value ${key}">
